...
//  memoryCardGame
//
//  Created by Sirarpi on 10/11/19.
...
class VCLLoggingViewController: UIViewController {
    ...
    private static func bumpInstatnceCount(for loggingName: String) ->Int {
    ...
    private func logVCL(_ msg: String) {
        if instanceCount == nil {
            instanceCount = VCLLoggingViewController.bumpInstatnceCount(for: vclLoggingName)
        }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
         logVCL("viewWillDissappear(animated =\(animated))")
    }
    override func viewDidDisappear(_ animated: Bool) {
        super.viewDidDisappear(animated)
         logVCL("viewDidDissappear(animated =\(animated))")
    }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (context: UIViewControllerTransitionCoordinatorContext!) ->Void in self.logVCL("begin animate(alongsideTransition:completion)")}, completion: {context -> Void in self.logVCL("end animate(alongsideTranslition:completion:)")})
    }
    /*
    // MARK: - Navigation
